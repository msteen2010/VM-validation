...
def get_protected_vms_latest_backup_href(data: str) -> str:
    ...
    try:
        ...
        for item in backups:
            if item:        # Checking to see if an entry is empty, if so, skip to the next entry
                # Checking to see if the backup is full as NetWorker can create app aware SQL VM image backups
                if item['level'] != 'Full':
                    level_found = item['level']
                    logger.error(f'Found a non-full backup of VM image: Level of backup found was {level_found}')
                    continue
